            ...
            try:
                ...
                if site.status_code == requests.codes.ok:
                    ...
                    if len(html_soup.find_all('div', class_='alert alert-info')) == 0:
                        self.az_logger.info(
                            'SuccessFul Get Request -> {} using Proxy -> {} on try-> {}'.format(url, proxy, flag))
                        flag = 0
                        return html_soup
                    else:
                        self.az_logger.debug(
                            'Recapta Detected -> {} using Proxy -> {} on try-> {}'.format(url, proxy, flag))
                        flag -= 1
                        if flag == 0:
                            return '0'
                else:
                    self.az_logger.debug(
                        'Proxy Status Mismatch -> {} using Proxy -> {} on Try -> {}'.format(site.status_code, proxy,
                                                                                            flag))
                    flag -= 1
                    if flag == 0:
                        return '0'
                ...
                self.az_logger.debug(
                    'Something Went Wrong -> {} using  Proxy -> {} Error -> {} on try-> {}'.format(url, proxy, E, flag))
                flag -= 1
                if flag == 0:
                    return '0'
        return '0'
                    ...
                    try:
                        ...
                        if html_soup != '0':
                            ...
                            for album in albums:
                                ...
                                self.az_data['albums_details'].append(
                                    {'id': album_id, 'singer_name': title, 'name': album_name, 'type': album_type,
                                     'year': album_year})
                            for song in songs:
                                song_href = song.a.get('href')
                                song_name = song.a.text
                                flag += 1
                                self.az_data['songs_details'].append(
                                    {'song_id': flag, 'singer_name': title, 'song_name': song_name,
                                     'song_href': song_href})
                        ...
                        self.az_logger.warning(
                            'Something Wrong while Requesting -> {} Error -> {} on line No. -> {}'.format(url, E,
                                                                                                          sys.exc_info()[
                                                                                                              -1].tb_lineno))
            temp_flag -= 1
    ...
    def lyrics_from_link(self, link):
        ...
        if html_soup != '0':
            try:
                ...
                song_name = \
                html_soup.find('div', class_='col-xs-12 col-lg-8 text-center').find_all('div', class_='div-share')[
                    1].text.split('"')[1]
                ...
                self.az_logger.warning(
                    'Error in Scrapping lyrics {} Lineno.->{}'.format(E, sys.exc_info()[-1].tb_lineno))
                return '0'
        else:
            return '0'
    ...
    async def get_batch_lyrics(self):
        ...
        while (flag):
            try:
                ...
                self.save_az_lyrics()
        self.az_logger.info(
            'Scrapped Successful... No. of Lyrics in file is  {}'.format(len(self.az_lyrics['completed'])))
        self.save_az_lyrics()
